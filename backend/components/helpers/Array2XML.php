...
class Array2XML extends BaseArrayHelper {
    ...
    public static function &createXML($node_name, $arr = array()) {
        ...
        $xml->appendChild(static::convert($node_name, $arr));
        ...
        if (is_array($arr)) {
            ...
            foreach($arr as $key=>$value){
                ...
                if (is_array($value) && is_numeric(key($value))) {
                    ...
                    foreach ($value as $v) {
                        $node->appendChild(static::convert($key, $v));
                    }
                } else {
                    // ONLY ONE NODE OF ITS KIND
                    $node->appendChild(static::convert($key, $value));
                }
    ...
    public static function removeParentNode($xml, $parentName) {
        $xml = strstr($xml, '<' . $parentName . '>');
        $xml = str_replace('<' . $parentName . '>', '', $xml);
        $xml = str_replace('</' . $parentName . '>', '', $xml);
